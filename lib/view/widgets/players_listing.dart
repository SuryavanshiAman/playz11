  ...
  Widget build(BuildContext context) {
    return Consumer2<PlayerViewModel, GameViewModel>(
      ...
      final announcementPlayers = playerData
          .where((player) => player.playigstatus.toString() == '1')
          .toList();
      final unannouncementPlayers = playerData
          .where((player) => player.playigstatus.toString() == '2')
          .toList();
      final allPlayers = playerData
          .where((player) => player.playigstatus.toString() == '0')
          .toList();
      teamIds =
          ...
          itemBuilder: (_, int i) {
            ...
            bool isSelectable = player.isSelected! || // Allow deselection
                (totalSelectedPlayers < maxTotalPlayers &&
                    (((player.designationName == "WK" && wkCount < maxPlayersPerRole) ||
                        (player.designationName == "BAT" && batCount < maxPlayersPerRole) ||
                        (player.designationName == "AR" && arCount < maxPlayersPerRole) ||
                        (player.designationName == "BOWL" && bowlCount < maxPlayersPerRole)) &&
                        ((player.teamId == teamIds[1] && !isMaxPlayersForTeam1) ||
                            (player.teamId == teamIds[0] && !isMaxPlayersForTeam2))));



            bool datat = playerProvider.canAddPlayer(player.designationName!);
            print("here $datat");

            if (!playerProvider.canAddPlayer(player.designationName!) && !player.isSelected!) {
              isSelectable = false;
            }
              ...
              child: ContainerConst(
                ...
                onTap: () {
                  if (isSelectable) {
                    ...
                    if (kDebugMode) {
                      print("Max players selected for the team. Cannot select more.");
                    }
                  }
                },
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            player.playigstatus.toString() == '0'
                                ? const SizedBox.shrink()
                                : TextConst(
                                    text: " • ",
                                    fontSize: 25,
                                    textColor: player.playigstatus.toString() ==
                                            '1'
                                        ? AppColor.activeButtonGreenColor
                                        : player.playigstatus.toString() == '2'
                                            ? AppColor.primaryRedColor
                                            : Colors.transparent,
                                  ),
                            Container(
