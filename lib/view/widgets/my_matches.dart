...
import 'package:playz11/new_pages_by_harsh/live_match/live_match_view.dart';
...
class MyMatchListing extends StatefulWidget {
  final List<GameData> matchData;

  const MyMatchListing({super.key, required this.matchData});
...
class MyMatchListingState extends State<MyMatchListing> {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      child: widget.matchData.isEmpty
          ?
      Utils.noDataAvailableVector(messageLabel: "Oops! No Data Found")
          : ListView.builder(
              ...
              shrinkWrap: true,
              itemBuilder: (_, int i) {
                final match = widget.matchData[i];
                final date =
                              ...
                              child: Row(
                                ...
                                children: [
                                  if (match.status == 2 ||
                                    ...
                                    TextConst(
                                      text:
                                          "${match.total_joined_team} Team • ${match.total_joined_contest} Contest",
                                      fontSize: Sizes.fontSizeZero,
                                      textColor: AppColor.textGreyColor,
                                    ),
                                  match.status == 3
