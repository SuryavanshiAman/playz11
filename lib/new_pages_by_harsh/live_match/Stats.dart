...
import 'package:playz11/new_pages_by_harsh/model/sate_model.dart';
import 'package:playz11/res/SharedPreferencesUtil.dart';
import 'package:playz11/res/app_url_const.dart';
...
class _LiveStatsState extends State<LiveStats> {
  ...
  Future<void> TeamStatesData() async {
    ...
    print('${AppApiUrls.getLiveTeamState}$gameId/$userToken/$matchId');
  ...
  Widget _buildPlayerItem(int index) {
    ...
    // Check if playerid matches any player in the selected team's playerlist
    final isMatchingPlayer = teamPlayerList.any(
      (teamPlayer) => teamPlayer.playerid == matchPlayer.playerid,
    );

    // Find the matching player, or return a default value if not found
    final matchingPlayer = teamPlayerList.firstWhere(
        (player) => player.playerid == matchPlayer.playerid,
        orElse: () =>
            Playerlist() // Create a default instance of Playerlist if not found
        );

    // Check if matchingPlayer is a valid instance to avoid null dereference
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            Column(
              children: List.generate(
                  // teamsData[selectedHighlight].playerlist?.length ?? 0,
  ...
  //       children: [
  //         TextConst(
  //           text: matchPlayer.totalPoint ?? "0",
  //           width: 65,
  //           textColor: AppColor.blackColor,
  //         ),
  //         TextConst(
  //           text: matchPlayer.selBy ?? "0%",
  //           textColor: AppColor.textGreyColor,
  //           width: 65,
  //         ),
  //         TextConst(
  //           text: matchPlayer.cBy ?? "0",
  //           textColor: AppColor.blackColor,
  //           width: 65,
  //         ),
  //         TextConst(
  //           text: matchPlayer.vcBy ?? "0",
  //           textColor: AppColor.textGreyColor,
  //           width: 65,
  //         ),
  //       ],
