...
import 'package:playz11/repo/update_apk_repo.dart';
...
class SplashServices with ChangeNotifier  {
  ...
  Future<void> updateApkApi(context) async {
    _updateApk.updateApkApi().then((value) {
      ...
      if (value['status'] == "200") {
        ...
        }else{ _checkUserSessionAndManageNavigation(context);}
      } else {
  ...
  void _downloadAndInstall(context, String downloadUrl) async {
    ...
    String filePath = "${dir?.path}/playz11.apk";
    ...
    InstallPlugin.installApk(filePath, appId: 'com.fc.playz11')
        .then((result) {
      _checkUserSessionAndManageNavigation(context);
      if (kDebugMode) {
